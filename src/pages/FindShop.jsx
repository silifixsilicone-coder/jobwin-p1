...
import { Search, MapPin, Loader2, Frown, Store } from 'lucide-react';
...
const FindShop = () => {
    ...
    const filteredShops = shops.filter(shop => {
        const matchesSearch = (
            ...
            shop.services?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        ...
        const matchesLocation = !locationFilter ||
            ...
            (shop.serviceCities && shop.serviceCities.some(city => city.toLowerCase().includes(locationFilter.toLowerCase())));
        ...
        <div className="bg-light-grey min-h-screen py-16">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="mb-14 flex items-center gap-6">
                    ...
                    <div>
                        <h1 className="text-4xl font-black text-[#1A1A1A] uppercase tracking-tighter">Business Directory</h1>
                        <p className="text-slate-500 font-medium">Connect with verified supply and fabrication shops nationwide.</p>
                    </div>
                </div>
                ...
                <div className="bg-[#1A1A1A] p-6 rounded-md shadow-2xl flex flex-col md:flex-row gap-4 mb-14 border border-white/5">
                    <div className="flex-1 relative">
                        ...
                        <input
                            type="text"
                            placeholder="Search shops or fabrication services..."
                ...
                {loading ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                        {filteredShops.map((shop, index) => (
                            ...
                            >
                                <ServiceCard service={shop} type="shop" />
                            </motion.div>
                        ))}
                    </div>
                ) : (
                    <div className="text-center py-32 bg-white rounded-md border border-slate-200 shadow-sm border-dashed">
                        ...
                        <h3 className="text-2xl font-black text-[#1A1A1A] mb-3 uppercase tracking-tighter">No Businesses Listed</h3>
                        <p className="text-slate-400 font-medium">Register your shop to start serving customers.</p>
                    </div>
                )}
            </div>
        </div>
